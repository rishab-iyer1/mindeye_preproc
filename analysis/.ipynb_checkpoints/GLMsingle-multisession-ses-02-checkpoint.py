...
# bids directory
homedir = '/jukebox/norman/rsiyer/rtmindeye/data/bids'
...
if make_avg_mask == 1:
    ...
    for run in range(1,n_runs+1):
        if run < 10:
            mask_name = func_dir + sub + '_ses-02_task-study_run-0%i_space-T1w_desc-brain_mask.nii.gz' % (run)
        else:
            mask_name = func_dir + sub + '_ses-02_task-study_run-%i_space-T1w_desc-brain_mask.nii.gz' % (run)
        mask_imgs.append(mask_name)
